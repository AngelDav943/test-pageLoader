    ...
    async function loadQuery(req, res) {
        ...
        try {
            ...
            if (fs.existsSync(scriptpath)) {
                let dirpath = `${config.path}${urlpath.toLowerCase().substring(0, urlpath.length - url[url.length-1].length)}`
                ...
                if (fs.existsSync(scriptpath)) {
                    ...
                    scriptloaded = true
                    // return
                } else if (urlpath.toLowerCase != "/" && url[0].toLowerCase() != "assets") {
                    scriptpath = `${config.path}/${url[0].toLowerCase()}/notfound.js`
